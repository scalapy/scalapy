...
object Writer extends TupleWriters {
  implicit def pyObjWriter[T <: Any]: Writer[T] = new Writer[T] {
  ...
  implicit val pyDynamicObjWriter: Writer[Dynamic] = new Writer[Dynamic] {
    override def write(v: Dynamic): Either[PyValue, Any] = {
      Right(v)
    }
  }
